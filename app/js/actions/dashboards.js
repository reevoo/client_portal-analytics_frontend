...
export const getProfileAndDashboards = () => (dispatch, getState) => {
  ...
  getProfile().then((profile) => {
    dispatch({ type: actionTypes.GET_PROFILE_SUCCESS, payload: profile })
    ...
    getWorkbooks(dashboardIds).then((dashboards) => {
...
export const changeFilter = (filterName, filterValue) => (dispatch, getState) => {
  ...
  if (filter) {
    setFilterValue(getCurrentWorkbook(state), filter, filterValue).then(() => dispatch({
